...
export default function CashForecast() {
  ...
  const forecastChartData = CASH_FORECAST_DATA.map((item, idx) => {
    let opt = item.optimized;
    if (delayPayables) opt += 15000;
    if (surgeExpenses && idx > 2) opt -= 8000;
    return {
      ...item,
      displayOptimized: opt
    };
  });
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          ...
          <h3 className="text-2xl font-extrabold text-slate-900 mt-1">{CASH_OVERVIEW.runwayMonths} Months</h3>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          ...
          <h3 className="text-2xl font-extrabold text-teal-800 mt-1">
            {(CASH_OVERVIEW.runwayMonths + (delayPayables ? 0.8 : 0.4)).toFixed(1)} Months
          </h3>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          ...
          <h3 className="text-2xl font-extrabold text-emerald-700 mt-1">
            ${(forecastChartData[7].displayOptimized).toLocaleString()}
          </h3>
