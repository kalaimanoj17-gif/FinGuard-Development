...
export default function SettingsPage() {
  ...
  useEffect(() => {
    api.getSettings()
      .then((res) => {
        if (res.businessProfile) setProfile(res.businessProfile);
        if (res.settings) {
          setSensitivity(res.settings.sensitivity ?? 85);
          setAutoApproveMatches(res.settings.autoApproveMatches ?? true);
          setEmailAlerts(res.settings.emailAlerts ?? true);
        }
      })
      .catch(() => {});
  }, []);
