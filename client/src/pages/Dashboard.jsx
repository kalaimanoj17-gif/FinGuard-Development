...
export default function Dashboard({ onSelectAction, setActiveTab }) {
  const [data, setData] = useState({
    ...
    cash: CASH_OVERVIEW,
    invoices: INVOICE_OVERVIEW,
    expenses: EXPENSE_OVERVIEW,
    security: FRAUD_SECURITY_DATA
  });
  ...
  const fetchDashboardData = () => {
    api.getDashboard()
      .then(res => {
        if (res) {
          setData({
            business: res.business || BUSINESS_PROFILE,
            health: res.financialHealth || FINANCIAL_HEALTH_SCORE,
            cash: res.cashOverview || CASH_OVERVIEW,
            invoices: res.invoiceOverview || INVOICE_OVERVIEW,
            expenses: res.expenseOverview || EXPENSE_OVERVIEW,
            security: {
              activeAlertsCount: res.activeActions ? res.activeActions.length : 1
            }
          });
        }
      })
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-3">
        <div className="flex items-center justify-between border-b border-slate-100 pb-2">
          <span className="text-xs font-bold text-slate-900 flex items-center gap-2">
            ...
            Financial Health Score Factors ({data.health.score} / 100 • {data.health.status} Risk)
          </span>
          <span className="text-[11px] text-slate-400 font-mono">Real-time Risk Audit</span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
          {(data.health.factors || FINANCIAL_HEALTH_SCORE.factors).map((f, idx) => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title="Cash Position"
          value={`₹${(data.cash.totalCash || 185000).toLocaleString('en-IN')}`}
          change={`+₹${(data.cash.netCashFlowMonth || 35000).toLocaleString('en-IN')}`}
          isPositive={true}
          subtitle="net cashflow this mo."
          icon={Wallet}
          accentColor="emerald"
          badge={`${data.cash.runwayMonths || 7.2} Mo Runway`}
          aiInsight="Cash buffer healthy. 7.2 months operational runway based on current burn."
        />

        <StatCard
          title="Outstanding Invoices"
          value={`₹${(data.invoices.pendingAmount || 0).toLocaleString('en-IN')}`}
          change={`₹${(data.invoices.overdueAmount || 0).toLocaleString('en-IN')} Overdue`}
          isPositive={false}
          subtitle={`${data.invoices.pendingCount || 0} pending invoices`}
          icon={FileText}
          accentColor="teal"
          badge={`${data.invoices.pendingCount || 0} Pending`}
          aiInsight="Real-time collection analytics active on registered customer accounts."
        />

        <StatCard
          title="Monthly Expenses"
          value={`₹${(data.expenses.totalMonth || 185000).toLocaleString('en-IN')}`}
          change="Live Sync"
          isPositive={false}
          ...
          accentColor="blue"
          badge="Under Budget"
          aiInsight={data.expenses.spendInsight || "Inventory spending monitored in real time."}
        />

        <StatCard
          title="Alerts & Security"
          value={`${data.security.activeAlertsCount || 0} High Risk`}
          change="AI Guarded"
          isPositive={true}
          subtitle="prevented loss"
          icon={AlertTriangle}
          accentColor="red"
          badge={`${data.health.score}% Health`}
          aiInsight="FinGuard AI Agent active — scans for duplicate vendor claims and GST deadlines."
        />
      </div>
