...
export default function FraudAlerts({ onSelectAction }) {
  const [fraudData, setFraudData] = useState(FRAUD_SECURITY_DATA);
  ...
  const fetchFraudAlerts = () => {
    api.getFraudAlerts()
      .then(res => {
        if (res) {
          setFraudData({
            securityPostureScore: res.securityPostureScore ?? FRAUD_SECURITY_DATA.securityPostureScore,
            riskLevel: res.riskLevel ?? FRAUD_SECURITY_DATA.riskLevel,
            activeAlertsCount: res.activeAlertsCount ?? FRAUD_SECURITY_DATA.activeAlertsCount,
            duplicateInvoicesCount: res.duplicateInvoicesCount ?? 1,
            highRiskAlert: res.highRiskAlert || FRAUD_SECURITY_DATA.highRiskAlert,
  ...
  useEffect(() => {
    fetchFraudAlerts();
    const interval = setInterval(fetchFraudAlerts, 5000);
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          ...
          <h3 className="text-2xl font-extrabold text-slate-900 mt-1">{fraudData.securityPostureScore}/100</h3>
          <span className="text-[11px] text-amber-700 font-semibold">{fraudData.riskLevel}</span>
        </div>
        ...
        <div className="bg-red-50/50 p-5 rounded-2xl border border-red-200 shadow-xs">
          ...
          <h3 className="text-2xl font-extrabold text-red-800 mt-1">{fraudData.activeAlertsCount} High Risk</h3>
          <span className="text-[11px] text-red-700 font-medium">{fraudData.duplicateInvoicesCount || 1} Duplicate Inv Flagged</span>
        </div>
